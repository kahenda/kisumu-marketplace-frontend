    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <input
          ...
          placeholder="Search listings e.g. jacket, shoes, CBD..."
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-8 grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <select value={filters.condition} onChange={e => setFilters({...filters, condition: e.target.value})}
          ...
          {CONDITIONS.filter(c => c).map(c => <option key={c} value={c}>{c.replace('_', ' ')}</option>)}
        </select>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map(listing => (
            <Link key={listing.id} to={`/listings/${listing.id}`} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition group">
              <div className="bg-gray-100 h-48 flex items-center justify-center text-4xl">🧥</div>
              <div className="p-3">
                <h3 className="font-semibold text-gray-900 text-sm truncate group-hover:text-orange-500">{listing.title}</h3>
                <p className="text-orange-500 font-bold mt-1">KES {Number(listing.price)?.toLocaleString()}</p>
